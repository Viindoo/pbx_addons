...
class Settings(models.Model):
    ...
    def _get_instance_data(self):
        ...
        for rec in self:
            ...
            rec.api_key = self.env['ir.config_parameter'].sudo().get_param('asterisk_plus.api_key')
    ...
    def register_instance(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param(
            'asterisk_plus.api_key', res['api_key'])
    ...
    def unregister_instance(self):
        ...
        res = self.make_api_request(urljoin(self.get_param('api_url'), 'registration'),
            requests.delete,
            headers={'x-api-key': api_key}
        )
        if not res and self.get_param('api_fallback_url'):
            logger.warning('Making a request to API fallback.')
            res = self.make_api_request(urljoin(self.get_param('api_fallback_url'), 'registration'),
            requests.delete,
            headers={'x-api-key': api_key},
            raise_on_error=True
        )
        self.env['ir.config_parameter'].set_param('asterisk_plus.api_key', '')
